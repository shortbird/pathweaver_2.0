...
When Spark students complete assignments, their work automatically populates their Optio portfolio. Students submit once in Spark, and it appears in both platforms instantly.
...
**Step-by-step:**
1. Student clicks "View Optio Portfolio" button in Spark
2. Spark generates a JWT token (signed with a shared secret)
3. Spark redirects student to: `https://optioeducation.com/spark/sso?token={jwt}`
4. Optio validates the token and logs the student in
5. Student lands on their Optio dashboard
...
### What It Does
Optio admins can sync your courses/assignments to automatically create Optio quests/tasks. Students then complete those tasks by submitting in Spark (via webhooks from Feature 2).
...
We'll exchange two secrets via secure channel (1Password, LastPass, or encrypted email):

1. **SSO Secret** (`OPTIO_SHARED_SECRET`) - For signing JWT tokens
2. **Webhook Secret** (`OPTIO_WEBHOOK_SECRET`) - For signing webhook payloads
...
## Feature 4: Account Linking (Optional Enhancement)

### What It Does
Students who already have Optio accounts can link them to their Spark account, so all their Spark progress appears in their existing portfolio.

### Two Scenarios

#### Scenario 1: Student Already Has Optio Account

**Flow:**
1. Student has been using Optio independently (created account at optioeducation.com)
2. Student now starts using Spark and tries to SSO
3. Optio detects existing account by email address
4. Optio automatically links Spark ID to existing account
5. All Spark progress now syncs to their existing portfolio

**No action needed:** This works automatically if emails match.

#### Scenario 2: SSO First, Then Want Direct Login

**Flow:**
1. Student SSOs from Spark (creates Optio account automatically)
2. Student discovers they can access Optio directly
3. Student goes to Optio Settings → Security → "Set Password"
4. Student can now log in to optioeducation.com with email/password
5. Both Spark SSO and direct login work simultaneously

**No action needed:** Optio handles this automatically.

### Email Matching Logic (Already Built into SSO)

When a student SSOs from Spark:

```javascript
// Optio's SSO endpoint does this automatically:
1. Check if lms_integrations has spark_user_id → use that account
2. Else, check if users table has matching email → link to that account
3. Else, create new user account
```

**Result:** Students can create Optio accounts first, or SSO first - either way works seamlessly.

### What If Emails Don't Match?

**Problem:** Student has Optio account with `student@personal.com` but Spark uses `student@school.edu`

**Solution Options:**

**Option A: Student Updates Email (Recommended)**
1. Student logs into Optio directly
2. Goes to Settings → Account → Update Email
3. Changes to school email
4. Next SSO from Spark links automatically

**Option B: Manual Linking (Requires Optio Support)**
1. Student contacts support@optioeducation.com
2. Provides both email addresses
3. Optio team manually links accounts in database
4. Future SSOs work automatically

### Do You Need to Build Anything?

**No!** This all works automatically with the SSO implementation from Feature 1.

The key is:
- Always use the same email in JWT `email` claim that the student uses in Spark
- Optio handles account matching/creation/linking automatically

### Edge Case: Student Has Two Optio Accounts

If a student accidentally creates two accounts (one direct, one via SSO with different emails):

**Optio's Solution:**
1. Student contacts Optio support
2. Optio merges the accounts (combines all quest progress, XP, badges)
3. Student can use either email to log in

**No Spark action needed.**

---

## Summary: What You're Building

1. **SSO:** Generate JWT, redirect to Optio (~20 lines)
2. **Webhooks:** POST submission data when student completes assignment (~50 lines)
3. **Course API:** Two read-only endpoints for courses and assignments (~100 lines)
4. **Account Linking:** Already works automatically via email matching (0 lines - just use correct email in JWT)