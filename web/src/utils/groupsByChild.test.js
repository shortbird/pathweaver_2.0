/**
 * The same cases as mobile/src/utils/__tests__/groupsByChild.test.ts, so the
 * two surfaces cannot drift apart silently.
 *
 * The shapes are taken from the report that prompted the split: a parent of
 * three (Zayah, Daxton, Rivers) with 37 class chats, three of which shared the
 * name "Elementary Microschool (Wednesday) Parent Chat" and three more the name
 * "Peak Play PE Parent Chat".
 */
import { describe, it, expect } from 'vitest'
import { groupsByChild, classMeetingLabel, studentName, isChildClassGroup } from './groupsByChild'

const kid = (id, first_name) => ({ id, first_name, last_name: 'T', display_name: first_name })

const grp = (id, name, students = [], meeting = null) => ({
  id,
  name,
  member_count: 5,
...
describe('groupsByChild', () => {
  ...
  it('preserves the server ordering (recency) inside a section', () => {
    ...
    const groups = [
      grp('newest', 'A', [d]), grp('older', 'B', [d]), grp('oldest', 'C', [d]),
      grp('x', 'D', [kid('z', 'Zayah')])
    ]
  ...
  it('handles an empty list', () => {
    ...
    expect(groupsByChild(undefined).sectioned).toBe(false)
  })
})
...
describe('isChildClassGroup', () => {
  it('is true only for a group the backend attributed to a child', () => {
    expect(isChildClassGroup(grp('g', 'Chat', [kid('z', 'Zayah')]))).toBe(true)
    expect(isChildClassGroup(grp('g', 'Chat'))).toBe(false)
    expect(isChildClassGroup(null)).toBe(false)
  })
})
