/**
 * The task template library, and the editor behind it.
 *
 * Any task, one step or many, can be saved as a template (iCreate meeting
 * 2026-09-23); this is where they are kept, edited and synced onto the tasks
 * already assigned from them. It was the checklist template library until
 * checklists became multi-step tasks on 2026-09-24.
 *
...
const TemplateEditor = ({ orgId, template, onSaved, onCancel }) => {
  ...
  const save = async () => {
    ...
    if (!cleaned.length) { toast.error('Add at least one step'); return }
    setBusy(true)
    try {
  ...
  return (
    <div className="border border-optio-purple/30 rounded-xl p-4 space-y-3 bg-optio-purple/5">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Template name (e.g. Employee onboarding)" className={inputClass} aria-label="Template name" />
      ...
      <label className="block">
        <span className="block text-xs font-medium text-neutral-500 mb-1">
          Directions (optional), shown above the steps
        </span>
        <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={2}
          placeholder="Directions for completing this task"
      ...
      {items.map((it, i) => (
        <div key={i} draggable
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm text-neutral-600">
            ...
            <label className="flex items-center gap-1.5"
              title="Adds an Upload button to the step. To hand them a document instead, use the link field above.">
...
/** The task template library: a single collapsed row until opened.
 * Authoring is the rare act; it must not sit on top of the daily list. */
export const TaskTemplatesManager = ({ orgId, onChanged, embedded = false, onCount, open: externalOpen }) => {
  ...
  useEffect(() => {
    if (query.isError) toast.error('Failed to load templates')
  }, [query.isError])
  ...
  const syncTemplate = async (t) => {
    const assigned = assignmentsQuery.data
      ...
      : null
    const who = assigned === null ? 'the tasks already assigned from it'
      : `${assigned} assigned task${assigned === 1 ? '' : 's'}`
    if (!(await confirm(
      `Update ${who} to match "${t.name}"? Finished tasks are left alone, and `
    ...
    try {
      ...
      const skipped = d.skipped_complete
        ? `, ${d.skipped_complete} finished task${d.skipped_complete === 1 ? '' : 's'} left alone` : ''
      toast.success(parts.length
        ? `${d.synced} task${d.synced === 1 ? '' : 's'} updated (${parts.join(', ')})${skipped}`
        : `Everything already matches${skipped}`)
      load()
      onChanged?.() // the sync rewrote assigned checklists — the list is stale
    } catch (err) {
      toast.error(err?.response?.data?.error || 'Could not sync the tasks')
    }
  }
    ...
    <>
      ...
      {embedded && templatesOpen && (
        <div className="flex items-center justify-between gap-3 mb-2">
          <span className="text-sm font-semibold text-neutral-800">Task templates</span>
          <button onClick={() => setEditing('new')} className="text-sm text-optio-purple font-medium hover:underline">
            + New template
          </button>
        </div>
            ...
            <li key={t.id} className="py-2.5 flex items-center gap-2 flex-wrap">
              ...
              <div className="flex-1 min-w-0 flex items-center gap-2 flex-wrap">
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full ${t.audience === 'family' ? 'bg-optio-pink/10 text-optio-pink' : 'bg-optio-purple/10 text-optio-purple'}`}>
                  {t.audience === 'family' ? 'Family' : t.audience === 'student' ? 'Student' : 'Staff'}
                </span>
                {t.role_type && <span className="text-xs px-2 py-0.5 rounded-full bg-gray-100 text-neutral-600">{t.role_type}</span>}
                <span className="text-xs text-neutral-400">{(t.items || []).length} {(t.items || []).length === 1 ? 'step' : 'steps'}</span>
                {t.description && (
      ...
      {editing && (
        <ModalOverlay onClose={() => setEditing(null)}>
          <div className="bg-white rounded-xl shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto p-5 space-y-4"
            role="dialog" aria-modal="true" aria-label="Task template">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold text-neutral-900">
                {editing === 'new' ? 'New task template' : `Edit "${editing.name}"`}
              </h2>
