/**
 * Block registry: the block-type catalogue and the constants that describe how
 * each one looks.
 *
 * Split out of ./index.js, which also re-exports the block editor components.
 * CalloutBlockEditor and DividerBlockEditor read CALLOUT_VARIANTS and
 * DIVIDER_STYLES from here, and used to read them from ./index -- which imports
 * those same two editors. A module importing its own barrel is a cycle, and ES
 * modules resolve one by handing whichever module loads second a
 * partially-initialised copy of the first. It worked because the constants
 * happened to be evaluated before the re-exports; nothing guaranteed that.
 *
 * The rule this file exists to keep: a barrel may import from its members, or
 * its members may import from it, but not both. Constants live here, the barrel
 * re-exports them, and editors import from here directly.
 */
import {
  DocumentTextIcon,
  PhotoIcon,
