...
const StatusBanner = ({ goal }) => {
  if (!goal || goal.status === 'draft') {
    return (
      <div className="rounded-lg bg-gray-100 text-gray-600 text-sm px-4 py-3 mb-4">
  ...
  if (goal.status === 'submitted') {
    return (
      <div className="rounded-lg bg-gradient-primary text-white text-sm px-4 py-3 mb-4">
      ...
      {active && form && (
        <div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5 mb-4">
            <h2 className="text-base font-semibold text-gray-900 mb-1">Direction</h2>
            <p className="text-sm text-gray-500 mb-3">The big picture — where is {firstName(active.name)} heading after this?</p>
            ...
            <input
              ...
              disabled={!!preview}
            />
            <label className="block text-sm font-medium text-gray-700 mt-3 mb-1">Notes (optional)</label>
            <textarea
              className={field}
              rows={3}
          ...
          {form.subjects.map((s, i) => (
            <div key={s.subject} className="bg-white rounded-xl border border-gray-200 p-5 mb-4">
              <h2 className="text-base font-semibold text-gray-900 mb-3">{s.subject}</h2>
              <label className="block text-sm font-medium text-gray-700 mb-1">
          ...
          <div className="flex flex-wrap items-center justify-end gap-3 pb-8">
            ...
            {preview ? (
              <span className="text-sm text-gray-400 italic">Preview — parents fill this in and submit for review</span>
            ) : (
              <>
