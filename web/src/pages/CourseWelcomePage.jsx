...
/**
 * CourseWelcomePage — where a student lands from the course enrollment email
 * sent when a partner program registers them for a purchased course. Sets the
 * password through the same token endpoint as password reset, which also
 * confirms the email address: one link, one step, ready to log in.
 *
 * Deliberately not a temporary password in the email — see
 * backend/utils/invite_tokens.py.
 */
const CourseWelcomePage = () => {
  ...
  // Server-resolved invite details. Query params are only a first-paint
  ...
  useEffect(() => {
    ...
    api.get(`/api/auth/invite-info/${token}`)
      ...
      .catch((err) => {
        ...
        // 404 (unknown/used token) surfaces on submit; the page still renders.
      })
  }, [token])
      ...
      <div className="max-w-md w-full space-y-8">
        <div className="text-center">
          ...
          <h2 className="mt-6 text-3xl font-bold text-gray-900">
            {firstName ? `Welcome to Optio, ${firstName}` : 'Welcome to Optio'}
          </h2>
          <p className="mt-3 text-sm text-gray-600">
            {orgName ? `${orgName} set up` : 'We set up'} your Optio account and
            added your course. Choose a password to finish setting it up.
          </p>
          {email && (
          ...
          <div className="bg-red-50 border border-red-200 rounded-md p-4">
            ...
            {(errorMessage.includes('expired') || errorMessage.includes('used')) && (
              <p className="mt-2 text-sm text-red-800">
                ...
                with your email address to get a fresh link.
              </p>
            )}
          </div>
        )}
