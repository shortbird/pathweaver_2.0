...
/**
 * The ledger side of Billing: Charges, Outstanding, Monthly tuition and
 * Charge detail, one body switched by `view` -- the shell (BillingPage)
 * owns the tab bar and the URL since M23; until then this WAS the Billing
 * page, with its own pill bar and the view in local state. The four views
 * share their modals (pay, refund, receipt, invoice), the search box and
 * the household list, which is why they are one component rather than four.
 *
 * `recurring`/`loadRecurring` come from the shell (useRecurringTuition),
 * which also needs the count for the tab's badge -- one fetch, not two.
 */
...
const LedgerPanel = ({ view, recurring, loadRecurring }) => {
  ...
  // invoice queue from, so this is their route to a first charge.
  ...
  return (
    <div>
      ...
      {/* ── Monthly tuition ─────────────────────────────────────────────
          A school billing a monthly rate has no invoice until the first month
          is charged, so Charges and Outstanding are both empty while real
          money is scheduled; without this tab the page said the school bills
          nothing. The Add button lived on the Tuition page until M23, and this
          tab told people to go there for it. */}
      {view === 'monthly' && (
        <div>
          ...
          <div className="flex items-start justify-between gap-4 mb-4">
            <p className="text-sm text-neutral-500 max-w-2xl">
              ...
              family, however many children they have — so anything below still marked as
              waiting has not been charged yet.
            </p>
