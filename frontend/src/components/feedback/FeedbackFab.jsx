...
/**
 * Beta feedback FAB for staff (iCreate pilot).
 *
...
const FeedbackFab = ({
  ...
  panelPosition = 'bottom-20 right-4',
}) => {
  ...
  const submit = async () => {
    ...
    try {
      await api.post('/api/bug-reports', {
        ...
        platform: platform || (surface === 'sis' ? 'web-sis' : 'web'),
        extra: {
          report_type: type,
          surface,
  ...
  return (
    <>
      {/* FAB — compact circle tucked into the corner so it never covers page controls */}
      <button
        onClick={() => setOpen((v) => !v)}
        aria-label="Send beta feedback"
        title="Send beta feedback"
        className={`fixed ${buttonPosition} z-[90] flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-r from-optio-purple to-optio-pink text-white shadow-lg hover:opacity-90 transition`}
      >
      ...
      {open && (
        <div className="fixed inset-0 z-[91]" onClick={reset}>
          <div
            className={`absolute ${panelPosition} w-80 bg-white rounded-2xl shadow-2xl border border-gray-100 p-4`}
