...
/**
 * CreateClassModal - Modal for creating a new SIS class.
 *
 * Collects: name, description, image, meeting days (Mon-Fri), start time,
 * duration (minutes), capacity (max students), optional supply fee, and an age range.
 *
 * onSubmit is called with (classData, imageFile). The image is uploaded by the
 * caller after the class is created (it needs the new class id).
 */
const DAY_OPTIONS = [
  { code: 'mon', label: 'Mon' },
  { code: 'tue', label: 'Tue' },
...
export default function CreateClassModal({ onClose, onSubmit }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      max_students: numOrUndef(formData.max_students),
