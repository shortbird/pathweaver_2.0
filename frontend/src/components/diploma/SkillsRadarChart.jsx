...
const CustomAngleTick = ({ payload, x, y, cx, cy, compact }) => {
  ...
  const info = competencyInfo[payload.value];
  if (!info) return null;

  // Calculate position relative to center
  const angle = Math.atan2(y - cy, x - cx);
  const distance = compact ? 25 : 40;
  const iconX = cx + Math.cos(angle) * (Math.sqrt((x - cx) ** 2 + (y - cy) ** 2) + distance);
  const iconY = cy + Math.sin(angle) * (Math.sqrt((x - cx) ** 2 + (y - cy) ** 2) + distance);

  return (
    <g>
      <foreignObject
        x={iconX - (compact ? 12 : 16)}
        y={iconY - (compact ? 12 : 16)}
        width={compact ? 24 : 32}
      ...
      <div>
        <ResponsiveContainer width="100%" height={280}>
          <RadarChart data={radarData} margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
            ...
            <PolarAngleAxis
              dataKey="competency"
              tick={(props) => <CustomAngleTick {...props} compact={true} />}
            />
            <PolarRadiusAxis
              angle={90}
              domain={[0, maxXP]}
      ...
      <div className="flex items-center justify-center mb-8">
        <ResponsiveContainer width="100%" height={500}>
          <RadarChart data={radarData} margin={{ top: 60, right: 80, bottom: 60, left: 80 }}>
            ...
            <PolarAngleAxis
              dataKey="competency"
              tick={(props) => <CustomAngleTick {...props} compact={false} />}
            />
            <PolarRadiusAxis
              angle={90}
              domain={[0, maxXP]}
