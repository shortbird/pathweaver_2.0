...
export default function PaperworkTemplatesManager({
  ...
  onChanged,
  initiallyOpen = false,
  // Forwarded to FormBuilder so a caller can open straight into a new form
  // (Task Center's "New form template" action).
  initialEditing = null,
}) {
  const [open, setOpen] = useState(initiallyOpen || !!initialEditing)
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className={open ? 'mt-3 pt-3 border-t border-gray-100' : 'hidden'}>
        <div className={activeTab === 'forms' ? '' : 'hidden'}>
          <FormBuilder orgId={orgId} staff={staff} embedded initialEditing={initialEditing}
            open={open && activeTab === 'forms'} onCount={setFormsCount} />
        </div>
