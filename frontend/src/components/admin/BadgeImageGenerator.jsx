...
const BadgeImageGenerator = () => {
  ...
  const fetchBadges = async () => {
    try {
      ...
      if (filter === 'missing') {
        badgesData = badgesData.filter(b => !b.image_url)
      }
  ...
  const handleGenerate = async (regenerate = false) => {
    ...
    const confirmed = window.confirm(
      `This will generate teen-focused images for ${count} badge${count > 1 ? 's' : ''} using ${count} API calls (${apiUsage.remaining - count} will remain).\n\nContinue?`
    )
    ...
    try {
      ...
      const response = await api.post('/api/badges/admin/batch-generate-images', {
  ...
  const getMissingCount = () => {
    return badges.filter(b => !b.image_url).length
  }
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className="text-2xl font-bold mb-2">Badge Image Generator</h2>
        <p className="text-gray-600">Generate teen-focused images for badges using AI-enhanced search</p>
        <p className="text-sm text-gray-500 mt-1">
          Images are searched with "teenage teen student" prefix for age-appropriate visuals
        </p>
      </div>
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-gray-600">
            ...
            {selectedBadges.size > 0 && ` = ${selectedBadges.size} API calls`}
          </span>
        </div>
        <div className="flex gap-2">
          <button
            ...
            className="bg-gradient-primary text-white px-4 py-2 rounded-lg hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed font-medium"
          >
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-gray-50 rounded-lg">
          <p className="text-gray-600">
            {filter === 'missing' ? 'All badges have images!' : 'No badges found'}
          </p>
        </div>
      ) : (
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Badge Name</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Identity Statement</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {badges.map((badge) => (
                <tr
                  key={badge.id}
                  className={`hover:bg-gray-50 ${selectedBadges.has(badge.id) ? 'bg-purple-50' : ''}`}
                >
                  ...
                  <td className="px-4 py-3 text-sm text-gray-500">
                    {badge.image_generated_at
                      ? new Date(badge.image_generated_at).toLocaleDateString()
                      : '-'}
                  </td>
                </tr>
              ))}
