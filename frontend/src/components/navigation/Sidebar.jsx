...
const Sidebar = ({ isOpen, onClose, isCollapsed, isPinned, onTogglePin, isHovered, onHoverChange }) => {
  ...
  // Add Classes link for students enrolled in at least one class
  ...
  if (user?.role === 'admin' || user?.role === 'superadmin') {
    // Only add Parent link if not already added via parent relationships
    if (!hasParentRelationships) {
      navItems.push({
        name: 'Parent',
        path: '/parent/dashboard',
  ...
  // Add Showcase link for superadmin or users with the can_view_showcase flag
  if (user?.role === 'superadmin' || user?.can_view_showcase === true) {
    navItems.push({
      name: 'Showcase',
      path: '/showcase',
      icon: (
