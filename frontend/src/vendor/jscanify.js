...
/*
 * Vendored from https://github.com/puffinsoft/jscanify (src/jscanify.js).
 * The npm package's entry point is the Node build, which hard-depends on
 * node-canvas + jsdom, and the browser build is UMD reading a bare global
 * `cv` — neither survives Vite. So: UMD wrapper replaced with an ES module
 * whose OpenCV instance is injected via setOpenCv() (the bare `cv`
 * references below resolve to the module-scope variable). The class body is
 * otherwise unmodified — diff against upstream when updating.
 */

let cv = null;

/** Inject the initialized OpenCV.js module before constructing a scanner. */
export function setOpenCv(instance) {
  cv = instance;
}
