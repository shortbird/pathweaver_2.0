 ...
 *
 * @param {string} studentId - The student/child ID to fetch data for
...
export function useParentChildOverview(studentId) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await api.get(`/api/parent/child-overview/${studentId}`);
      ...
      const transformed = {
        ...
        // For PortfolioSection - use portfolio_achievements which includes evidence from in-progress quests
      ...
      logger.error('Error fetching child overview:', err);
      setError(err.response?.data?.error || 'Failed to load child overview');
    } finally {
      setIsLoading(false);
    }
  }, [studentId]);
