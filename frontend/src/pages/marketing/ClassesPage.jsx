...
const Hero = ({ onClaim }) => {
  ...
  return (
    <section ref={ref} className="relative overflow-hidden bg-gradient-primary text-white pt-32 pb-20 sm:pt-40 sm:pb-28">
...
const SubjectCatalog = ({ onClaim }) => {
  ...
  return (
    <section ref={ref} id="subjects" className="py-16 sm:py-20 bg-white scroll-mt-24">
...
// Horizontal travel (px) that counts as a swipe rather than a tap or a scroll.
...
const PhoneCarousel = () => {
  ...
  const go = (dir) =>
    setIndex((i) => (i + dir + APP_SCREENSHOTS.length) % APP_SCREENSHOTS.length)

  const handleTouchStart = (e) => {
    setPaused(true)
    const t = e.touches[0]
    swipe.current = { x: t.clientX, y: t.clientY, dx: 0, dy: 0 }
  }

  const handleTouchMove = (e) => {
    if (!swipe.current) return
    const t = e.touches[0]
    swipe.current.dx = t.clientX - swipe.current.x
    swipe.current.dy = t.clientY - swipe.current.y
  }

  // Only treat it as a swipe if the gesture was mostly horizontal — otherwise the
  // visitor was scrolling the page with a finger that happened to start on the phone.
  const handleTouchEnd = () => {
    const s = swipe.current
    swipe.current = null
    if (!s) return
    if (Math.abs(s.dx) > SWIPE_THRESHOLD_PX && Math.abs(s.dx) > Math.abs(s.dy)) {
      go(s.dx < 0 ? 1 : -1)
    }
  }
  ...
  return (
    ...
    >
      <div
        className="relative w-[240px] sm:w-[280px] touch-pan-y select-none focus:outline-none focus-visible:ring-2 focus-visible:ring-optio-purple focus-visible:ring-offset-4 rounded-[2.75rem]"
        role="group"
        tabIndex={0}
        aria-roledescription="carousel"
        aria-label={`App screenshots — swipe or use arrow keys. Showing ${index + 1} of ${APP_SCREENSHOTS.length}`}
      ...
      >
        ...
        {/* Swipe affordance — mobile only, where there is no hover to hint at interactivity. */}
        <p className="md:hidden mt-3 text-center text-xs text-gray-400 font-medium">
          Swipe to see more
        </p>
      </div>
...
const HowItWorks = () => {
  ...
  const track = useCtaTracker(PAGE)
  return (
    <section ref={ref} className="py-16 sm:py-20 bg-gradient-to-b from-gray-50 to-white">
          ...
          <div className="order-2 md:order-1 space-y-6 sm:space-y-8 max-w-md mx-auto md:mx-0 md:ml-auto">
            {HOW_IT_WORKS.map((s) => (
              <div key={s.step} className="flex items-start gap-5">
                <div className="flex-shrink-0 w-12 h-12 rounded-full bg-gradient-to-br from-optio-purple to-optio-pink text-white flex items-center justify-center text-lg font-bold shadow-md">
                  {s.step}
                </div>
                <p className="text-lg text-gray-800 leading-snug pt-2.5 font-medium">
                  {s.text}
                </p>
              </div>
            ))}
...
const TranscriptMockup = () => (
  <div className="relative max-w-sm mx-auto md:mx-0">
    <div className="bg-white rounded-xl shadow-2xl ring-1 ring-gray-200 overflow-hidden">
      ...
      {/* Student info */}
      <div className="px-6 py-3 border-b border-gray-100 text-xs text-gray-600 flex justify-between">
        <span><span className="text-gray-400">Student: </span>Jordan L.</span>
        <span><span className="text-gray-400">Grade: </span>11</span>
      </div>

      {/* Column headers */}
      <div className="px-6 pt-3 pb-2 grid grid-cols-[1fr_auto_auto] gap-4 text-[10px] uppercase tracking-wider text-gray-400 font-semibold">
        <span>Class</span>
        ...
        {TRANSCRIPT_ROWS.map((c, i) => (
          ...
          >
            <span className="text-gray-800 font-medium flex items-center gap-2 flex-wrap">
              {c.name}
              {c.optio && (
                <span className="px-2 py-0.5 bg-optio-pink text-white text-[9px] font-bold rounded-full tracking-wider">
                  OPTIO
                </span>
              )}
            </span>
            <span className={`text-gray-800 text-right w-10 ${c.optio ? 'font-bold' : 'font-semibold'}`}>{c.grade}</span>
...
const FULL_TIME_POINTS = [
  'A WASC-accredited diploma, not just individual class credit',
  'A dedicated Optio teacher, or lead the learning yourself as the parent',
  'Same passion-project approach — every class, all four years',
]

// Classes are the entry point; some visitors are actually shopping for a school.
// This hands those families off to /academy without diluting the free-class CTA.
const FullTimeCta = () => {
  const ref = useSectionView('full_time', PAGE)
  const track = useCtaTracker(PAGE)
  return (
    <section ref={ref} className="pb-16 sm:pb-20 bg-white">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="rounded-3xl bg-gradient-primary text-white p-8 sm:p-12 shadow-xl">
          <div className="md:flex md:items-center md:gap-12">
            <div className="md:flex-1">
              ...
              <p className="text-lg text-white/90 leading-relaxed mb-6">
                Optio Academy is our fully online private school. Instead of transferring a class
                or two back to your local high school, your student earns their entire accredited
                diploma with us — built the same way, around work they actually care about.
              </p>
              <ul className="space-y-2.5 mb-8 md:mb-0">
                {FULL_TIME_POINTS.map((point) => (
            ...
            <div className="md:flex-shrink-0">
              <Link
                to="/academy"
                onClick={() => track('full_time_academy')}
                className="btn-inverse btn-lg w-full md:w-auto"
              >
                Explore Optio Academy
              </Link>
            </div>
          </div>
        </div>
...
const FinalCta = ({ onClaim }) => {
  ...
  const track = useCtaTracker(PAGE)
  return (
    <section ref={ref} className="py-16 sm:py-20 bg-gradient-primary text-white">
