...
const Check = ({ on, label }) => (
  <span aria-label={`${label}: ${on ? 'yes' : 'no'}`} className={on ? 'text-green-600 font-semibold' : 'text-gray-300'}>
    {on ? 'Yes' : '—'}
  </span>
)
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        <div className="flex items-center gap-4 min-w-0">
          {student.avatar_url ? (
            ...
            <div className="w-14 h-14 rounded-full bg-gradient-primary text-white flex items-center justify-center text-lg font-semibold flex-shrink-0">
            ...
            <dl className="space-y-2">
              {profileKeys.map((k) => (
                <div key={k} className="grid grid-cols-3 gap-3 text-sm">
                  <dt className="text-gray-500">{prettyKey(k)}</dt>
                  <dd className="col-span-2 text-gray-800 whitespace-pre-wrap">{String(profile[k])}</dd>
                </div>
              ))}
            </dl>
          )}
          ...
          : (
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {fields.map((f) => (
                  <tr key={f.key}>
                    <td className="py-2 pr-3 text-gray-500 whitespace-nowrap">{f.label}</td>
                    <td className="py-2 pr-3 text-gray-800">{assessments[f.key]?.boy || '—'}</td>
                    <td className="py-2 text-gray-800">{assessments[f.key]?.eoy || '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <Card title="Curriculum materials">
        {materials.length === 0
          ? <p className="text-sm text-gray-400">No materials listed yet.</p>
          : (
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-xs text-gray-400 border-b border-gray-100">
              ...
              <tbody className="divide-y divide-gray-50">
                {materials.map((m) => (
                  <tr key={m.id}>
                    <td className="py-2 pr-3 text-gray-800">{m.item_name}</td>
          ...
          : scores.map((c) => (
            <div key={c.class_id || c.class_name} className="mb-5 last:mb-0 break-inside-avoid">
              <div className="flex items-baseline justify-between gap-3 mb-1.5">
                <h3 className="text-sm font-semibold text-gray-800">{c.class_name}</h3>
                {c.average != null && (
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-50">
                  {(c.assignments || []).map((a) => (
                    <tr key={a.id}>
                      <td className="py-1.5 pr-3 text-gray-800">{a.name}</td>
                      <td className="py-1.5 pr-3 text-gray-500">{a.date_completed || '—'}</td>
