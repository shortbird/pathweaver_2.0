-- Complete Database Migration: Delete Old Schema and Create V3 Schema
...
DROP TRIGGER IF EXISTS trigger_user_skill_xp_updated_at ON user_skill_xp;
DROP TRIGGER IF EXISTS trigger_learning_logs_updated_at ON learning_logs;
...
DROP FUNCTION IF EXISTS get_monthly_active_users();
...
CREATE TABLE quests (
    ...
    title TEXT NOT NULL,
    description TEXT,
    big_idea TEXT, -- Main quest description
...
-- Quest Tasks table (V3 - replaces old XP awards)
...
CREATE TABLE user_quest_tasks (
    ...
    UNIQUE(user_id, quest_task_id) -- Prevent duplicate completions
);
...
CREATE POLICY "Public profiles viewable" ON users
    FOR SELECT USING (true); -- Allow viewing for diploma pages, etc.
...
-- Clean up and revert the XP service to use new pillar keys since database now supports them
COMMENT ON TABLE user_skill_xp IS 'V3 table using new pillar_type enum with proper constraints';
COMMENT ON TABLE quest_tasks IS 'V3 task system with per-task XP and pillar assignment';
COMMENT ON TABLE user_quest_tasks IS 'V3 task completion tracking with evidence';