...
Provides the quest calendar and completed quests for parents.
Part of parent/dashboard.py refactoring (Month 6 - Backend Optimization).

GET /quest/<student_id>/<quest_id> -- the parent-shaped read of one quest --
was removed on 2026-09-15. A parent reads the child's quest from the child's
own route, GET /api/quests/<id>?student_id=, which answers with the student's
full payload (utils/guardian_scope); this one had drifted to a thinner copy
(no big_idea, no journal moments, no class credit ring).
"""
