...
def register_routes(bp):
    ...
    def create_course(user_id):
        ...
        try:
            ...
            if not can_create_course(user_data):
                return jsonify({'error': 'Not authorized to create courses'}), 403
    ...
    def update_course(user_id, course_id: str):
        ...
        try:
            ...
            course_result = client.table('courses').select(
                'created_by, organization_id, status, title, slug'
            ...
            allowed_fields = [
                ...
                # Transcript credit shown by the partner-enrollment flow
                'credit_subject', 'credit_amount',
            ]
            ...
            if 'status' in updates and get_effective_role(user_data) != 'superadmin':
                if updates['status'] != 'draft':
                    return jsonify({
                        'error': 'Only an admin can publish or archive a course'
                    }), 403
    ...
    def upload_quest_header_image(user_id, quest_id: str):
        ...
        try:
            ...
            # Check if user can manage by quest creator or via a course they own
            if not can_manage_course(user_data, quest):
                # Also check if quest belongs to a course the user created
                course_link = client.table('course_quests').select('course_id, courses!inner(created_by)').eq('quest_id', quest_id).limit(1).execute()
                course_for_quest = course_link.data[0]['courses'] if course_link.data else None
                if not can_manage_course(user_data, {'created_by': course_for_quest.get('created_by')} if course_for_quest else None):
                    return jsonify({'error': 'Insufficient permissions. Only the course creator or superadmin can manage courses.'}), 403
