-- Quest Template System Migration (FIXED)
...
-- =====================================================
-- PHASE 0: Fix pillar constraints on ALL tables
-- =====================================================

-- Drop the old check constraints if they exist
ALTER TABLE quest_tasks 
DROP CONSTRAINT IF EXISTS quest_tasks_pillar_check;

ALTER TABLE user_skill_xp
DROP CONSTRAINT IF EXISTS user_skill_xp_pillar_check;

-- Add new check constraints with both old and new pillar values (for transition)
ALTER TABLE quest_tasks
ADD CONSTRAINT quest_tasks_pillar_check CHECK (
  pillar IN (
    -- Old pillars (for existing data)
    'creativity', 
    'critical_thinking', 
    'practical_skills', 
    'communication', 
...
ALTER TABLE user_skill_xp
ADD CONSTRAINT user_skill_xp_pillar_check CHECK (
  pillar IN (
    -- Old pillars (for existing data)
    'creativity', 
    'critical_thinking', 
...
-- Add foreign key for path_id in quest_metadata (if not exists)
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'fk_quest_metadata_path'
  ) THEN
...
-- 3.1 Update existing quest_tasks with new pillar values
UPDATE quest_tasks 
SET pillar = CASE pillar
  WHEN 'creativity' THEN 'arts_creativity'
  WHEN 'critical_thinking' THEN 'stem_logic'
  WHEN 'practical_skills' THEN 'life_wellness'
  WHEN 'communication' THEN 'language_communication'
  WHEN 'cultural_literacy' THEN 'society_culture'
  ELSE pillar
END
WHERE pillar IN ('creativity', 'critical_thinking', 'practical_skills', 'communication', 'cultural_literacy');

-- 3.2 Update user_skill_xp table with new pillar values
...
DROP TRIGGER IF EXISTS update_mastery_on_xp_change ON user_skill_xp;
...
-- =====================================================
-- PHASE 8: Clean up - remove old constraint after migration
-- =====================================================

-- After verifying all data is migrated, update the constraints to only allow new values
ALTER TABLE quest_tasks 
DROP CONSTRAINT IF EXISTS quest_tasks_pillar_check;
...
SELECT DISTINCT pillar, COUNT(*) FROM quest_tasks GROUP BY pillar;

-- Check user mastery
SELECT COUNT(*) as users_with_mastery FROM user_mastery;

-- Check new tables
SELECT table_name FROM information_schema.tables 
WHERE table_schema = 'public' 
AND table_name IN ('quest_metadata', 'quest_paths', 'quest_customizations', 'user_badges', 'user_mastery', 'pillar_subcategories');
*/