-- ============================================================
-- Migration: Recreate Collaboration Tables
-- Date: 2026-01-05
-- Description: Recreates collaboration infrastructure tables
--              that were previously deleted
-- ============================================================

-- Table: quest_collaborations
-- Stores collaborative quest instances with creator and org tracking
...
-- Tracks which users are part of each collaboration group
...
-- Tracks peer approvals for shared evidence with response tracking
...
CREATE INDEX IF NOT EXISTS idx_quest_collaborations_quest_id
    ON quest_collaborations(quest_id);

CREATE INDEX IF NOT EXISTS idx_quest_collaborations_organization_id
    ON quest_collaborations(organization_id);

CREATE INDEX IF NOT EXISTS idx_quest_collaboration_members_collaboration_id
    ON quest_collaboration_members(collaboration_id);

CREATE INDEX IF NOT EXISTS idx_quest_collaboration_members_user_id
    ON quest_collaboration_members(user_id);

CREATE INDEX IF NOT EXISTS idx_shared_evidence_collaboration_id
    ON shared_evidence(collaboration_id);

CREATE INDEX IF NOT EXISTS idx_shared_evidence_submitted_by
    ON shared_evidence(submitted_by);

CREATE INDEX IF NOT EXISTS idx_shared_evidence_approvals_shared_evidence_id
    ON shared_evidence_approvals(shared_evidence_id);

CREATE INDEX IF NOT EXISTS idx_shared_evidence_approvals_user_id
    ON shared_evidence_approvals(user_id);
