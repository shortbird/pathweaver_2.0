...
# get_real_ip moved to utils/client_ip.py so `utils` could call it without
# importing middleware (see that module). Re-exported: every existing
# `from middleware.rate_limiter import get_real_ip` keeps working, and there is
# still exactly one implementation of the X-Forwarded-For hop counting.
from utils.client_ip import get_real_ip  # noqa: F401
