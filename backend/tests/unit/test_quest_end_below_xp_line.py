...
class TestBelowTheLine:
    ...
    def test_an_ordinary_quest_is_set_aside_not_refused(self, client, mock_verify_token):
        ...
        resp, webhook, lti = _end(client, tables)
        ...
        assert written['is_active'] is False and written['status'] == 'set_down'
        assert 'completed_at' not in written, 'set aside must not count as finished'
...
class TestAtOrAboveTheLine:
    ...
    def test_an_ordinary_quest_completes_as_before(self, client, mock_verify_token):
        ...
        resp, webhook, _ = _end(client, tables)
        assert resp.status_code == 200
        body = resp.get_json()
