/**
 * The accredited-diploma credit arithmetic.
 *
 * Moved here from web/src/utils/creditRequirements.js unchanged. It is pure
 * business logic over shared/data/credits.json: no DOM, no network, no store,
 * nothing that belongs to one surface. The web app is its only caller today
 * and the mobile app is not required to grow one — what this move buys is that
 * if mobile ever shows a credit total, there is no second implementation for it
 * to disagree with, which is exactly how the requirement TABLE ended up with
 * four copies saying two different things about Social Studies.
 *
 * The backend does the conversion half of this in Python, over the same JSON
 * (backend/generated/credits.py). The overflow rule below has no backend
 * counterpart: it is a reading of a transcript, and only the clients present
 * it.
 */
...
/**
 * Keyed by subject, the shape the callers have always used. `xpRequired` is
 * derived rather than stored -- it is credits * XP_PER_CREDIT and nothing else,
 * and storing it is how a table gets a row where the two disagree.
 */
export const CREDIT_REQUIREMENTS: Record<string, SubjectCredit> =
  Object.fromEntries(
    CREDIT_REQUIREMENTS_DATA.map((c: CreditRequirement) => [
      c.key,
      {
        displayName: c.transcriptName,
        credits: c.credits,
        xpRequired: c.credits * XP_PER_CREDIT,
 ...
 */
export const calculateCreditsFromXP = (xp: number): number => {
 ...
 */
export const calculateTotalCredits = (subjectXP: Record<string, number>): number => {
 ...
 */
export const getCreditProgress = (subject: string, xp: number): any => {
  const requirement = CREDIT_REQUIREMENTS[subject];
 ...
 */
export const getAllCreditProgress = (subjectXP: Record<string, number>): any[] => {
 ...
 */
export const formatCredits = (credits: number | string): string => {
 ...
 */
export const getCreditStanding = (subjectXP: Record<string, number> = {}): any => {
 ...
 */
export const splitCreditProgress = (progress: any[] = []): { remaining: any[]; complete: any[] } => {
 ...
 */
export const creditsRemaining = (progress: any[] = []): number => Math.round(
 ...
 */
export const creditsBeyondRequirement = (credit: any): number => Math.round(
 ...
 */
export const meetsGraduationRequirements = (subjectXP: Record<string, number>): boolean => {
