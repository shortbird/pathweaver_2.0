...
- **This is a Vite APP, not a packaged design system.** There is no library build that
  ...
  `web/.ds-entry.jsx` (re-exports the barrel + the 4 unexported primitives so they
  land on `window.OptioUI`). `--entry ./web/.ds-entry.jsx`.
- **The entry MUST live inside `web/`** (a package.json with a `name`). The repo-root
  ...
  `web/` makes PKG_DIR = `web` (named `optio-quests-frontend`).
- **CSS is generated, not shipped.** Run **`bash .design-sync/gen-styles.sh` BEFORE
  `package-build.mjs`** on every (re)sync — it compiles `web/.ds-styles.css` (gitignored)
...
- **`@types/react`** isn't in `web/node_modules` by default; copied in from `.ds-sync`
  to strengthen prop extraction. On a fresh clone, `cp -R .ds-sync/node_modules/@types/react web/node_modules/@types/react`.
...
- **`PhilosophySection` is buggy** — references undefined `Clock`/`TrendingUp`/`Heart` (it
  ...
  is synced. Previews pass heroicons as the `icon` prop (heroicons resolve from web/node_modules).
...
- **Generated CSS is not committed.** If `gen-styles.sh` isn't run first, the build uses a
  stale/missing `web/.ds-styles.css`. Always regenerate.
...
- If `web/src/components/ui/**` changes, rebuild AND regenerate the CSS.
