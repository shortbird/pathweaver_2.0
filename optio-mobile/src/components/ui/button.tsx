...
const variantActionClasses: Record<ButtonVariant, Record<ButtonAction, string>> = {
  solid: {
    ...
    secondary: 'bg-surface-200 active:bg-surface-100',
...
export function Button({
  ...
  const baseClasses = `flex-row items-center justify-center gap-2 ${sizeClasses[size]} ${variantActionClasses[variant][action]}`;

  return (
    <ButtonContext.Provider value={{ variant, action, size }}>
      <Pressable className={`${baseClasses} ${className}`} {...props}>
        {children}
      </Pressable>
