# Brevo Email Copy — Free Class Nurture + Segment Templates

Voice: friendly, positive, concise, helpful. Not overly excited, not wordy. Plain and personal,
signed **Dr. Bowman**, sent from tanner@optioeducation.com with a live reply-to. No emojis.
**No em dashes.** "Class" language, not "credit" (credit only where mechanically necessary).

Positioning rules:
- **App-first.** Primary CTA is downloading the iOS/Android app (official store badge buttons,
  hosted in the Brevo gallery). Web gets a brief mention, never the headline.
- **Students create their class.** The flow is: choose a name, choose the subject you want to earn
  credit for, then personalize assignments (called tasks) by adding your own or using AI
  suggestions. Never "browse" or "pick from a catalog."
- **$149 per class** after the free one.
- **Never mention the possibility of a school not accepting the transfer.** No refund framing.
- Free-class sequence can talk transcripts and accreditation. **K-12 family emails (Families, POE,
  newsletter) lead with features instead**: portfolio, supporters, quests, custom classes, the app.

CTA URLs:
- iOS: https://apps.apple.com/us/app/optio-education/id6773061928
- Android: https://play.google.com/store/apps/details?id=com.optioeducation.optio
- Web (secondary): https://www.optioeducation.com/register (UTMs per plan doc)

Badge images (Brevo-hosted):
- App Store: https://img.mailinblue.com/11613506/images/rnb/original/6a4d5b9a3cb1d1ad25463b47.png
- Google Play: https://img.mailinblue.com/11613506/images/rnb/original/6a4d5b9b2adfaedf2a9521f7.png

Paste-ready HTML lives in `brevo_email_html/` (numbered to match Brevo template ids).

---

## Sequence: Free Class Nurture (trigger: added to "Free Class Leads"; exit: added to Customers)

### Email 1 — ~1 hour after signup
**Subject:** Getting your free class set up

Hey, Dr. Bowman here. I run Optio.

You grabbed a free class on our site. Here's how to get it:

1. Download the Optio app (links below).
2. Create your free account. It takes about a minute.
3. Create your class: choose a name, choose the subject you want to earn credit for, then personalize your assignments (we call them tasks) by adding them yourself or using AI suggestions.
4. Reply to this email and tell me the class you created. I'll make it your free class. No credit card, nothing to cancel.

[App Store badge] [Google Play badge]

Prefer a computer? Everything also works on the web at optioeducation.com. The app is the best way to use Optio, though.

Dr. Bowman

---

### Email 2 — day 2
**Subject:** You're probably already doing the work

Most students I talk to are already doing something that should count.

Training for a team? That's PE. Practicing a language every day? That's a world language class. Building games, editing videos, baking, running a small business? Real classes too.

An Optio class doesn't add new work on top of your life. You create a class around what you already do, and Optio gives it structure and makes it count.

Your first one is free. Grab the app:

[App Store badge] [Google Play badge]

Dr. Bowman

---

### Email 3 — day 4
**Subject:** Will your school actually accept it?

The first question almost everyone asks: does an Optio class really count?

Yes. When you finish a class, we send your local high school an official transcript. It's issued through Optio Academy, which is accredited by the Western Association of Schools and Colleges (WASC), the same kind of accreditation your local public school has.

Your free class is still open. Create it in the app:

[App Store badge] [Google Play badge]

Questions about your specific school? Reply. I've walked a lot of families through this.

Dr. Bowman

---

### Email 4 — day 7
**Subject:** What an Optio class actually looks like

No lectures. No worksheets. Here's the actual shape of it:

You create your class and fill it with real projects, done at your own pace. As you go, you capture evidence right from your phone: photos, videos, writing, the work itself. It all builds into a portfolio you can show people. An Optio teacher reviews everything before the class counts.

You can also invite supporters. Parents, grandparents, and mentors can see your progress and cheer you on.

The point isn't to race through it. It's to spend your time on work you're proud of, and have it count.

Start with the free one:

[App Store badge] [Google Play badge]

Dr. Bowman

---

### Email 5 — day 10
**Subject:** The details your parents will ask about

If you need to get a parent on board, forward them this. The short version:

- The first class is completely free. No credit card, nothing to cancel.
- After that, classes are $149 each. One-on-one support from an Optio teacher is available as an add-on.
- Every class is teacher-reviewed before it counts. The transcript is issued through Optio Academy, which is WASC-accredited.
- Everything runs in the Optio app (iOS and Android, plus the web), and parents can join as supporters to follow progress.

Parents: happy to answer anything directly. Just reply and it comes straight to me.

Dr. Bowman

---

### Email 6 — day 14
**Subject:** Should I keep your free class open?

I'll keep your free class available. No expiration games. But I don't want to keep emailing you if the timing isn't right.

If you're in, it's three steps: download the app, create your class, reply to tell me about it.

[App Store badge] [Google Play badge]

Stuck on something? Reply and I'll help.

And if now's not the time, no hard feelings. The unsubscribe link below works, and your free class will still be here if you come back.

Dr. Bowman

---

## One-off: Catch-up email (existing 8 free-class leads — send BEFORE automation goes live)
**Subject:** Your free Optio class (sorry for the wait)

Hey, Dr. Bowman here from Optio.

You asked for your first free class a little while back, and my reply is later than promised. That's on me. The ads worked better than my inbox did.

Your free class is still yours. Here's how to get it:

1. Download the Optio app (links below).
2. Create your free account, then create your class: choose a name and subject, and add your own tasks or use AI suggestions.
3. Reply and tell me the class you made. I'll make it your free class. No credit card, no commitment.

[App Store badge] [Google Play badge]

Questions? Just reply. I read and answer every message myself.

Dr. Bowman

*(After this sends, these contacts enter the automation at Email 2 timing — set LEAD_DATE so they skip Email 1.)*

---

## Segment: Families (parent-voiced welcome — manual send or 1-step automation)
**Subject:** Your questions about Optio, answered directly

Hi {FIRSTNAME}, Dr. Bowman here from Optio. Thanks for reaching out about your family.

Optio is built for the whole K-12 range. Your student creates classes around what they already love, does real projects, and captures the work as they go: photos, videos, writing. It all grows into a portfolio they can be proud of. You can join as a supporter, along with grandparents or mentors, to see their progress and cheer them on.

It all runs in the Optio mobile app (iOS and Android, with a web version too). For older students, classes are teacher-reviewed and can go on an official transcript. The first class is free.

The easiest way to see if it fits: download the app and have your student start something. Questions? Reply to this email and it comes straight to me.

[App Store badge] [Google Play badge]

Dr. Bowman

---

## Segment: B2B (demo/sales — personal template, sent manually; track as Brevo CRM deal)
**Subject:** Your Optio demo

Hi {{first name}},

Thanks for requesting a demo. I'd love to show you how Optio works and hear what you're building at {{organization}}.

Two options, whichever is easier:
- Grab a time here: {{scheduling link}}
- Or reply with a couple of windows that work and I'll send an invite.

If it's useful before we talk: Optio lets students turn self-directed projects into accredited classes and transcripts, and gives schools and programs the tools to run it (rosters, attendance, messaging, credit review).

Dr. Bowman

---

## Segment: POE Parents (one-shot, post-camp; parent emails only)
**Subject:** What your student started at POE doesn't have to stop

Hi {FIRSTNAME}, Dr. Bowman from Optio. Your student documented their Pipe Organ Encounter week with us this summer and put real work toward a half fine-arts class on an accredited transcript.

The same approach works for everything else they're into: sports, languages, coding, art. In the Optio app they create a class around what they already do, capture the work from their phone into a growing portfolio, and you can follow along as a supporter. The first class is free.

Want help setting up a class for fall? Reply to this email and it comes straight to me.

[App Store badge] [Google Play badge]

Dr. Bowman

---

## Monthly keep-warm newsletter (all non-converted lists)
Format: 3 short items, plain. (1) One real student project from that month (anonymized, with permission), (2) one new or seasonal class or quest idea, (3) one practical answer to a common family question. Always ends with: "Your first class is still free." Signed Dr. Bowman.
