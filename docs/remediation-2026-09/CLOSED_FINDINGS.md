...
| QF-02 | 15 components over 1,000 lines | All 15 split into 31 components. Bodies moved verbatim by script, then a babel scope pass caught six real mistakes | `frontend/src/__tests__/componentSize.test.js` — **`EXEMPT` is empty**; a fourth test fails when a file comes back under the cap and its exemption lingers |
| QF-03 | Two data-fetching paradigms in v1 | 8 highest-churn pages migrated to `hooks/api/`, which found three duplicate in-flight fetches and one staleness bug invisible to either file alone | `frontend/src/__tests__/dataFetchingParadigm.test.js` — ratchets **call sites**, not files, so a pure split does not move it |
...
| QF-07 | Styling drift | Ratcheted at 293 off-palette hex values, not converted — each is a visual change to a real screen | `frontend/src/__tests__/brandPalette.test.js` reads the sanctioned palette out of `tailwind.config.js`, so it cannot drift from the design system |
| QF-08 | a11y tooling installed with zero imports | axe wired over 10 shared UI primitives (leverage: they render on nearly every page). Clickable divs ratcheted at the real count, 142 | `frontend/src/components/ui/__tests__/a11y.test.jsx`. The test says out loud that axe catches maybe a third of real problems |
| QF-09 | v2 explicit `any` | Ratcheted at the real count, 580 (not the 390 estimated) | `frontend-v2/src/__tests__/typeWidening.test.ts` |
...
|---|---|---|---|
| SEC-04 | `.env.example` shipped the production Supabase project ref | Replaced with `your-project-ref` placeholders in `backend/` and `frontend/` | Never guarded. A one-line test asserting no tracked `.env.example` names `vvfgxcykxjybtvpfzwyx` would close this cheaply. **Verified clean 2026-09-08.** |
