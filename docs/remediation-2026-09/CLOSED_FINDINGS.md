...
| FU-05 | Acting-as tokens were body-only, so SEC-03's gate could not apply | `/act-as` set an httpOnly `acting_as_token` cookie and the body copy was gated. **Superseded 2026-09-15 by GAP-3 below**: the whole session was deleted, so there is no acting-as token to deliver | `tests/test_token_delivery.py` still scans every route for body credentials; `test_acting_as_cookie.py` went with the feature |
| GAP-3 | The parent "act as dependent" session (a swapped token for the child) outlived its replacement, family scope, as dead code on three surfaces | Deleted: `routes/dependents_acting_as.py`, the acting-as mint/verify/cookie half of `utils/session_manager.py`, `token_authority.is_acting_as_still_authorized`, `token_delivery.acting_as_body_tokens` (backend); `ActingAsContext`, `actingAsRestore`, `ActingAsBanner`, `beginSessionSwitch` (web); `startActingAs`/`stopActingAs`, the `'dependent'` mode, `ActingAsBanner`, the `forChildId` params (mobile). Admin masquerade, a different mechanism, stays | `test_parent_routes_registration.py::test_act_as_route_is_gone`; `test_verify_token_access_only.py` and `test_postgrest_role_claim.py` enumerate the minted token types (two impersonation types, not four); `test_auth_resolvers_fail_closed.py` pins `get_deescalation_user_id` to logout alone; `test_security_audit_regressions.py` proves a pre-deletion `acting_as_refresh` token re-mints nothing |
| BUG-2 (three clusters) | Whole features removed at the database and left in the code: the parent-connection request flow (`parent_connection_requests`, 8 calls), observer requests (`observer_requests`, 5) and the parent quest/evidence views (`user_quest_deadlines`, 2) | Deleted outright 2026-09-15: `routes/observer_requests.py`, `routes/parent/{quests_view,evidence_view}.py`, the three request routes in `routes/admin/parent_connections.py` and fourteen `ParentRepository` methods, with registrations and client methods | `test_dropped_tables_are_not_queried.py` ratcheted 33 -> 18 with its floor 20 -> 10; `test_client_api_paths_exist.py` for the client side |
