...
CREATE TABLE public.quest_collaborations (
  ...
  status text NOT NULL DEFAULT 'pending'::text CHECK (status = ANY (ARRAY['pending'::text, 'accepted'::text, 'declined'::text, 'completed'::text])),
  created_at timestamp with time zone DEFAULT now(),
...
CREATE TABLE public.quest_tasks (
  ...
  pillar text NOT NULL CHECK (pillar = ANY (ARRAY['creativity'::text, 'critical_thinking'::text, 'practical_skills'::text, 'communication'::text, 'cultural_literacy'::text])),
...
CREATE TABLE public.quests (
  ...
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  source text DEFAULT 'optio'::text,
...
CREATE TABLE public.submissions (
  ...
  status text,
  user_quest_id uuid,
  CONSTRAINT submissions_user_quest_id_fkey FOREIGN KEY (user_quest_id) REFERENCES public.user_quests(id)
);
...
CREATE TABLE public.user_quest_tasks (
  ...
  quest_task_id uuid,
  user_quest_id uuid,
  evidence_type text NOT NULL CHECK (evidence_type = ANY (ARRAY['text'::text, 'link'::text, 'image'::text, 'video'::text])),
...
CREATE TABLE public.users (
  ...
  subscription_tier character varying DEFAULT 'free'::character varying CHECK (subscription_tier::text = ANY (ARRAY['free'::character varying, 'basic'::character varying, 'premium'::character varying, 'enterprise'::character varying]::text[])),
