...
function BuddyMouth({
  ...
  if (feedReaction) {
    if (isChewing) {
      // Closed big satisfied smile while chewing
      const sw = 18 * scale;
      const sh = 6 * scale;
      return (
        <G>
          {/* Big closed grin */}
          <Path
            d={`M${cx - sw} ${cy} Q${cx} ${cy + sh * 2} ${cx + sw} ${cy}`}
            fill="#2A1A2E"
            stroke="#1A1A2E"
            strokeWidth={2.5}
            strokeLinecap="round"
          />
          {/* Rosy cheek bumps from smiling */}
          <Ellipse cx={cx - sw * 0.9} cy={cy + 2} rx={4 * scale} ry={3 * scale} fill="#FF8FAA" opacity={0.4} />
          <Ellipse cx={cx + sw * 0.9} cy={cy + 2} rx={4 * scale} ry={3 * scale} fill="#FF8FAA" opacity={0.4} />
        </G>
      );
    }

    // Wide smile open bite -- mouth open waiting for food
    const mw = 16 * scale;
    const mh = 12 * scale;
    return (
      <G>
        {/* Wide smile shape, open at bottom */}
        <Path
          d={`M${cx - mw} ${cy}
              Q${cx} ${cy - mh * 0.3} ${cx + mw} ${cy}
              Q${cx + mw * 0.7} ${cy + mh * 1.2} ${cx} ${cy + mh * 1.3}
              Q${cx - mw * 0.7} ${cy + mh * 1.2} ${cx - mw} ${cy} Z`}
          fill="#2A1A2E"
        />
        {/* Tongue at bottom */}
        <Ellipse cx={cx} cy={cy + mh * 0.8} rx={mw * 0.5} ry={mh * 0.3} fill="#FF8FAA" />
        {/* Top lip curve */}
        <Path
          d={`M${cx - mw - 2} ${cy} Q${cx} ${cy - mh * 0.35} ${cx + mw + 2} ${cy}`}
          fill="none"
          stroke="#1A1A2E"
          strokeWidth={1.5}
          strokeLinecap="round"
...
export default function OptioBuddy({
  ...
  const foodFlyVal = useRef(new Animated.Value(0)).current;
  const chewVal = useRef(new Animated.Value(1)).current;
  ...
  // ── Feed reaction: food flies in, then chew animation ──
  useEffect(() => {
    if (!feedReaction) { setShowFood(false); setIsChewing(false); return; }

    // Reset
    foodFlyVal.setValue(0);
    chewVal.setValue(1);
    setShowFood(true);
    setIsChewing(false);

    // Food flies from top-right to mouth, then chewing starts
    Animated.sequence([
      // Food flies in
      Animated.timing(foodFlyVal, {
        toValue: 1,
        duration: 400,
        easing: Easing.out(Easing.quad),
        useNativeDriver: false,
      }),
      // Food disappears, switch to chewing smile
      Animated.timing(foodFlyVal, {
        toValue: 2,
        duration: 100,
        useNativeDriver: false,
      }),
      // Trigger closed-mouth smile
      Animated.timing(chewVal, { toValue: 1, duration: 1, useNativeDriver: false }),
      // Chew 1
      Animated.timing(chewVal, { toValue: 0.92, duration: 150, useNativeDriver: false }),
      Animated.timing(chewVal, { toValue: 1.05, duration: 150, useNativeDriver: false }),
      // Chew 2
      Animated.timing(chewVal, { toValue: 0.93, duration: 130, useNativeDriver: false }),
      Animated.timing(chewVal, { toValue: 1.04, duration: 130, useNativeDriver: false }),
      // Chew 3
      Animated.timing(chewVal, { toValue: 0.94, duration: 120, useNativeDriver: false }),
      Animated.timing(chewVal, { toValue: 1.02, duration: 120, useNativeDriver: false }),
      // Settle
      Animated.timing(chewVal, { toValue: 1, duration: 200, useNativeDriver: false }),
    ]).start(() => setShowFood(false));
  }, [feedReaction]);

  // Map chewVal to body scale for chewing effect
  useEffect(() => {
    if (!feedReaction) return;
    const chewId = chewVal.addListener(({ value }) => {
      tapScaleVal.setValue(value);
    });
    // Switch to closed smile when food reaches mouth
    const foodId = foodFlyVal.addListener(({ value }) => {
      if (value >= 1.5 && !isChewing) {
        setIsChewing(true);
      }
    });
    return () => {
      chewVal.removeListener(chewId);
      foodFlyVal.removeListener(foodId);
    };
  }, [feedReaction, isChewing]);
  ...
  return (
    <TouchableOpacity onPress={onTap} activeOpacity={0.9}>
      <Svg viewBox="0 140 400 220" width={width} height={height}>
        ...
        {/* Flying food emoji */}
        {showFood && feedReaction && (() => {
          const foodEmojis: Record<string, string> = {
            crunch: '\u{1F34E}', sweet: '\u{1F36D}', spicy: '\u{1F336}',
            soupy: '\u{1F35C}', chewy: '\u{1F35E}', novel: '\u{1F363}',
          };
          const emoji = foodEmojis[feedReaction] || '\u{1F34E}';

          const foodX = foodFlyVal.interpolate({
            inputRange: [0, 1, 2],
            outputRange: [320, 200, 200],
          });
          const foodY = foodFlyVal.interpolate({
            inputRange: [0, 1, 2],
            outputRange: [160, 262, 262],
          });
          const foodOpacity = foodFlyVal.interpolate({
            inputRange: [0, 0.8, 1, 1.1, 2],
            outputRange: [1, 1, 1, 0, 0],
          });
          const foodScale = foodFlyVal.interpolate({
            inputRange: [0, 0.5, 1, 2],
            outputRange: [1.2, 1, 0.6, 0],
          });
