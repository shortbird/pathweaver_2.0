 ...
 *
 * ONE screen serves two viewers. A student opens their own quest; a parent
 * opens the same screen pointed at a child by passing `studentId`, and sees
 * what the child sees — the hero, the description, the rhythm calendar, every
 * task with its Definition of Done and its evidence, the class credit ring, and
 * the personalization wizard. Before this, the parent route was a separate,
 * thinner page that drifted: no big_idea, no journal moments, no class
 * progress, and task authoring only for under-13 dependents.
 *
 * What differs in parent mode is never the layout, only which endpoint a write
 * goes to and whether the control is offered at all. The backend answers a
 * delegated read with `viewer_context` (utils/guardian_scope) naming exactly
 * what this caller may do, so no button is rendered that a POST would refuse:
 *
 *   add a task      every verified child        → /api/family/quests/<q>/tasks
 *   add evidence    every verified child        → /api/evidence/helper/*
 *   complete a task managed dependents only     → /api/tasks/<t>/complete
 *   remove a task   managed dependents only     → /api/family/quests/<q>/tasks/<t>
 *
 * A student with their own login still marks their own work complete: that is
 * the point of the line, not an omission.
 */
...
/** "Added by Dad" under a block somebody else attached. Evidence can arrive
 *  from a parent or a teacher as well as the student, and an unlabelled photo
 *  in your own quest that you did not take is confusing on either side of the
 *  screen. Student-uploaded blocks get no label — that is the default case. */
function UploaderLabel({ block }: { block: any }) {
  const role = block.uploaded_by_role;
  if (role !== 'parent' && role !== 'advisor') return null;
  const who = block.uploaded_by_name || (role === 'parent' ? 'a parent' : 'a teacher');
  return (
...
function TaskItem({
  ...
  /** Writes the completion. Injected rather than done here so the parent
   *  on-behalf-of path lives in one place (useQuestDetail.completeTask). */
  onSubmitCompletion: (blocks: any[]) => Promise<void>;
  ...
  /** Parent mode: the child whose task this is. Evidence reads carry it and
   *  every write swaps to the on-behalf-of endpoint. */
  studentId?: string | null;
  /** Parent mode gates, straight from the delegated read's viewer_context.
  ...
  // Parent mode reads the CHILD's evidence document for this task; the backend
  // takes ?student_id= for verified guardians (utils/guardian_scope).
  const evidenceParams = studentId ? { params: { student_id: studentId } } : undefined;

  const refetchEvidence = async () => {
    if (task.is_moment) return;
    try {
      const { data } = await api.get(`/api/evidence/documents/${task.id}`, evidenceParams);
  ...
  }, [task.id, task.is_moment, studentId]);
  ...
  // Parent mode is exempt because the endpoint behind it is different: the
  // on-behalf-of completion records the parent's confirmation as the evidence,
  // and a parent ticking off something their kid did at the kitchen table has
  // nothing to attach.
  const needsEvidence = !task.is_moment && !studentId && evidenceBlocks.length === 0;
  ...
  // Parent mode: evidence is attached THROUGH the helper endpoints, which stamp
  // the block with who added it and leave the task itself a draft — the student
  // still owns marking it done. One batch call per save; the per-block endpoint
  // is the fallback for a backend that predates /batch.
  const saveHelperBlocks = async (newBlocks: any[]) => {
    const payload = newBlocks.map((b) => ({ block_type: b.type || b.block_type, content: b.content }));
    try {
  ...
  // Which blocks this viewer may take back off. A student owns everything on
  // their own task; a parent may only remove what they themselves attached —
  // the helper delete endpoint enforces exactly that, and offering an X on the
  // student's own photo would be a button that always fails.
  const canRemoveBlock = (block: any) =>
    !task.is_moment
    && (!studentId
      || (block.uploaded_by_role === 'parent' && block.uploaded_by_user_id === viewerUserId));
  ...
  const handleRemoveBlock = async (block: any) => {
    ...
    try {
      if (studentId) {
        await api.delete(`/api/evidence/helper/blocks/${block.id}`);
      } else {
  ...
  return (
    <Pressable onPress={() => setExpanded(!expanded)}>
              ...
              {!task.is_completed && (
                <VStack space="sm" className="mt-2">
                  ...
                  {canComplete ? (
                    ...
                    /* A student with their own login marks their own work done.
                       Say so where the button would be, rather than leaving a
                       gap that reads as something failing to load. */
                    <HStack className="items-center gap-1.5">
                      <Ionicons name="information-circle-outline" size={13} color={c.iconMuted} />
                      <UIText size="xs" className="text-typo-400 dark:text-dark-typo-400 flex-1">
                        Anything you add here is theirs to finish — they mark the task complete.
                      </UIText>
                    </HStack>
                  )}
                  {!task.is_required && canRemove && (
                    <Pressable
                      onPress={async (e) => {
                        e.stopPropagation();
                        const ok = await confirmAlert({
                          title: 'Remove task?',
                          message: `"${task.title}" will be removed from this quest. Any evidence added to it will be lost.`,
                          confirmText: 'Remove',
                          destructive: true,
                        });
      ...
      <TaskEvidenceSheet
        ...
        // Parent mode uploads through the helper routes, which need the student
        // and task in the body (the helper URL carries neither).
        uploadInitPath={studentId ? '/api/evidence/helper/upload-init' : undefined}
        uploadFinalizePath={studentId ? '/api/evidence/helper/upload-finalize' : undefined}
        extraInitBody={studentId ? { student_id: studentId, task_id: task.id } : undefined}
        extraFinalizeBody={studentId ? { student_id: studentId, task_id: task.id } : undefined}
        // Parent mode always overrides. For the student, only once the task is
        // finished: the sheet's own save posts 'draft', which would knock a
        // completed task back to unfinished.
        onSave={studentId
          ? (newBlocks) => saveHelperBlocks(newBlocks)
          : task.is_completed
...
export function QuestDetailView({ questId: id, studentId = null }: QuestDetailViewProps) {
  ...
  const c = useThemeColors();
  // What this viewer may do, decided by the backend on the delegated read. A
  // student on their own quest has no viewer_context and may do everything.
  const viewer: QuestViewerContext | null = quest?.viewer_context || null;
  const canComplete = !studentId || !!viewer?.can_complete_tasks;
  const canRemoveTasks = !studentId || !!viewer?.can_remove_tasks;
  // can_add_tasks is true for every child a guardian is verified against, but
  // read it rather than assume it — this is the flag the write endpoint checks.
  const canAddTasks = !studentId || viewer?.can_add_tasks !== false;
  ...
  useFocusEffect(
    React.useCallback(() => {
      // Never in parent mode: the global Capture button writes to whoever is
      // signed in, so scoping it to the CHILD's quest would file the parent's
      // capture against a quest the parent is not enrolled in.
      if (!studentId && quest?.id && quest?.title && !!quest?.user_enrollment) {
        ...
        <VStack className="max-w-4xl w-full md:mx-auto">
          <VStack className="px-5 md:px-8 pt-6 pb-12" space="lg">
            ...
            <VStack testID="quest-detail-header" space="sm">
              {/* Parent mode says whose quest this is before anything else. The
              ...
              <Card variant="elevated" size="lg" className="items-center">
                <VStack space="md" className="items-center w-full">
                  ...
                  <UIText size="sm" className="text-typo-500 dark:text-dark-typo-500 text-center">
                    {studentId
                      ? `Add this quest to ${childName || 'your kid'} and it shows up on their account, ready to personalize.`
                  ...
                  <Button size="lg" className="w-full" onPress={handleEnroll} loading={enrolling}>
                    <ButtonText>{studentId ? 'Start this quest for them' : 'Start Quest'}</ButtonText>
                  </Button>
                </VStack>
              </Card>
            )}
            ...
            {isEnrolled && (
              <VStack testID="quest-task-list" space="sm">
                  ...
                  <VStack space="sm">
                    {[...tasks].sort((a, b) => {
                      ...
                      <TaskItem
                        ...
                        onSubmitCompletion={(blocks) => completeTask(task.id, blocks).then(() => undefined)}
                        onDelete={deleteTask}
                        studentId={studentId}
                ...
                {/* Task creation wizard — the SAME personalization wizard in
                    both modes: same AI step, same complexity dial, same
                    write-your-own path. useQuestDetail swaps where an accepted
                    task is written; nothing here knows the difference. */}
                <TaskCreationWizard
                  questId={quest.id}
