...
## Code Quality & Audit-to-Fix Flow

| Command | When to Use |
|---------|-------------|
| `/full-audit` | Create 7 audit tasks → workers process |
| `/compile-audit` | After audits done, compile findings + save to JSON |
| `/fix-audit` | Create fix tasks from findings → workers implement |
| `/verify-fixes` | After fixes done, run tests and verify |
| `/queue-fix` | Add specific fix tasks manually (selective fixes) |
| `/queue-status` | See what's in the queue |
| `/quick-audit` | Fast pre-merge check (solo, no workers) |
| `/cleanup` | Clear completed tasks |

### Complete Audit-to-Fix Flow
```
T1: /full-audit           → Creates 7 audit tasks
T2-T4: /work-queue        → Workers run audits
[Workers say "Queue empty"]

T1: /compile-audit        → Scans code, saves findings
T1: /fix-audit            → Creates fix tasks from findings
T2-T4: /work-queue        → Workers implement fixes
[Workers say "Queue empty"]

T1: /verify-fixes         → Review what was fixed
```

### Iterative Fix Flow (add more fixes as needed)
```
T1: /queue-fix [describe what to fix]   → Adds task(s)
T1: /queue-status                        → See queue
T2-T4: /work-queue                       → Workers fix
...
### MULTI-AGENT COMMANDS (use workers in T2-T4)

```bash
# Audit + Fix Flow
T1: /full-audit        # Creates 7 audit tasks
T2-T4: /work-queue     # Workers run audits
T1: /compile-audit     # Compile findings
T1: /fix-audit         # Create fix tasks
T2-T4: /work-queue     # Workers implement fixes
T1: /verify-fixes      # Confirm fixes work

# Ship Feature Flow  
T1: /ship-feature [description]  # Creates 4 tasks
T2-T4: /work-queue               # Workers build it
T1: /integrate-feature           # Merge and PR

# Health Check (quick + optional deep)
T1: /health-check      # Quick checks + creates audit tasks
T2-T4: /work-queue     # Workers run deeper audits (optional)
```

### SOLO COMMANDS (single terminal)

```bash
/dev-session              # Morning startup
/quick-audit              # Fast pre-commit check
/debug-and-fix [error]    # Interactive debugging
/fix-production [error]   # Emergency hotfix
/sprint-start             # Create sprint plan
/release                  # Release process
/new-feature [idea]       # Feature planning (PRD)
/pr-reviewer [branch]     # Code review
/cleanup                  # Clear completed tasks
```
